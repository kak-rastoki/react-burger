...
import { CurrencyIcon, FormattedDate } from '@krgaa/react-developer-burger-ui-components';

import { useGetFeedQuery, useGetProfileOrdersQuery, useGetOrderByIdQuery } from '@/services/api/ordersApi';
...
export const OrderInfo = (): ReactElement | null => {
  ...
  const orderFromFeed: TOrder | undefined = feedData?.orders.find((o: TOrder): boolean => o._id === id);
  const orderFromProfile: TOrder | undefined = profileData?.orders.find((o: TOrder): boolean => o._id === id);
  const cachedOrder: TOrder | undefined = orderFromFeed || orderFromProfile;

  const { data: fallbackData, isLoading: isFallbackLoading } = useGetOrderByIdQuery(id!, {
    skip: !!cachedOrder || !id,
  });

  const order: TOrder | undefined = cachedOrder || fallbackData?.orders[0];

  if (isFallbackLoading || !order) {
    return (
      <div className={styles.container}>
        <p className="text text_type_main-medium text_color_inactive">Загрузка деталей заказа...</p>
      </div>
    );
  }
  ...
  const ingredientCounts: Record<string, number> = order.ingredients.reduce<Record<string, number>>((acc: Record<string, number>, ingId: string): Record<string, number> => {
    acc[ingId] = (acc[ingId] || 0) + 1;
    return acc;
  }, {});

  const uniqueIngredients: TIngredient[] = Object.keys(ingredientCounts)
    .map((ingId: string): TIngredient | undefined => allIngredients.find((item: TIngredient): boolean => item._id === ingId))
  ...
  return (
    <div className={styles.container}>
      ...
      <span className={`${styles.status} ${statusInfo.className} text text_type_main-default mb-15`}>
        {statusInfo.text}
      </span>
      ...
      <div className={`${styles.composition} mb-10`}>
        <ul className={styles.list}>
          {uniqueIngredients.map((ingredient: TIngredient): ReactElement => {
            const count: number = ingredient.type === 'bun' ? 2 : ingredientCounts[ingredient._id];
            return (
